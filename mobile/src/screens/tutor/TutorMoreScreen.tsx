...
import type { TutorMoreStackParamList } from '../../navigation/types';
import type { ThemeColors } from '../../theme/colors';
import { fonts } from '../../theme/fonts';
import { useTheme } from '../../theme/useTheme';

type Props = NativeStackScreenProps<TutorMoreStackParamList, 'TutorMoreHome'>;
...
// Same list and "already approved" filtering as MoreScreen.tsx's student
// version - a tutor can still apply to become a performer or sponsor too.
const ROLE_APPLICATIONS: { key: string; label: string; path: string; already: (user: PublicUser) => boolean }[] = [
...
// The tutor-mode More tab - deliberately its own lean menu rather than a
// reuse of the student MoreScreen: once you're in Tutor mode nothing from
// the student side should still be reachable underneath, per the "this is
// a full mode switch" instruction. "Switch to Student" is the one way
// back.
export default function TutorMoreScreen({ navigation }: Props) {
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
        ...
        {/* Reopens the same 14-screen content the mandatory first-visit
            gate shows (TutorDashboardScreen), but in review mode - closable
            at any point, doesn't re-log acceptance. TutorMoreScreen sits
            inside TutorMoreStack, itself inside TutorTabs, so it's two hops
            up to MainStack where TutorOrientation is registered. */}
        <MenuRow icon="school" label="Orientation & Policies" onPress={() => navigation.getParent()?.getParent()?.navigate('TutorOrientation', { reviewMode: true })} colors={colors} />
        <MenuRow icon="ribbon" label="Teacher Education" onPress={() => navigation.getParent()?.getParent()?.navigate('TeacherEducation')} colors={colors} />
...
function createStyles(colors: ThemeColors) {
  return StyleSheet.create({
    ...
    sheetTitle: { fontSize: 16, fontFamily: fonts.bodyBold, color: colors.text, marginBottom: 10 },
    sheetOption: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingVertical: 14, borderTopWidth: 1, borderTopColor: colors.border },
    sheetOptionText: { fontSize: 14.5, fontFamily: fonts.bodyMedium, color: colors.text },
    sheetEmptyText: { fontSize: 13, fontFamily: fonts.body, color: colors.textFaint, paddingVertical: 14, borderTopWidth: 1, borderTopColor: colors.border, textAlign: 'center' },
  });
}
