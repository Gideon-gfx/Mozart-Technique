...
import { TUTOR_ORIENTATION_SCREENS, TUTOR_ORIENTATION_VERSION } from '../../data/tutorOrientationContent';
import type { MainStackParamList } from '../../navigation/types';
import { fonts } from '../../theme/fonts';

type Props = NativeStackScreenProps<MainStackParamList, 'TutorOrientation'>;

// Mozart Techniques' own brand colors (mobile/src/theme/colors.ts's
// lightColors), not the theme hook - this modal deliberately alternates
// light/dark PER SCREEN for rhythm (the spec's own instruction), independent
// of the viewer's system light/dark preference, so these stay fixed rather
// than following useTheme().
const TOKENS = {
  dark: '#1a0505',
  darkDeep: '#0f0303',
  cardDark: '#2a0d0d',
...
const TOTAL = TUTOR_ORIENTATION_SCREENS.length;

export default function TutorOrientationScreen({ navigation, route }: Props) {
  ...
  const current = TUTOR_ORIENTATION_SCREENS[index];
  ...
  // (reopened from Tutor Settings) has nothing to gate, so back works normally.
  useEffect(() => {
  ...
  async function finish() {
    ...
    setSubmitting(true);
    try {
      await tutorsApi.acknowledgeTutorOrientation(TUTOR_ORIENTATION_VERSION);
      // Re-checks the session so user.needsTutorOrientation flips to false
      // immediately - without this, going back to the dashboard would
      // trigger the gate again on the very next focus.
  ...
  return (
    <View style={[styles.screen, { backgroundColor: bg }]}>
      ...
      >
        {TUTOR_ORIENTATION_SCREENS.map((screen) => {
