...
type Props = NativeStackScreenProps<PerformerMoreStackParamList, 'PerformerMoreHome'>;
...
// The performer-mode More tab - same lean, own-menu treatment every other
// mode's More tab gets (nothing from Student mode stays reachable
// underneath). History (past selected/completed gigs) lives here as a
// pushed screen, per the tab set this mode was built to.
export default function PerformerMoreScreen({ navigation }: Props) {
    ...
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      <View style={styles.profileRow}>
        ...
        <View style={styles.profileInfo}>
          ...
          <Text style={styles.role}>Performer</Text>
        </View>
      </View>
      ...
      <View style={styles.card}>
        <MenuRow icon="chatbubbles" label="Event messages" onPress={() => navigation.navigate('PerformerMessages')} colors={colors} />
        <MenuRow icon="time" label="History" onPress={() => navigation.navigate('PerformerHistory')} colors={colors} />
        ...
        {/* Reopens the same 14-screen content the mandatory first-visit
            gate shows (PerformerRequestsScreen), but in review mode -
            closable at any point, doesn't re-log acceptance. PerformerMoreScreen
            sits inside PerformerMoreStack, itself inside PerformerTabs, so
            it's two hops up to MainStack where PerformerOrientation lives. */}
        <MenuRow icon="school" label="Orientation & Policies" onPress={() => navigation.getParent()?.getParent()?.navigate('PerformerOrientation', { reviewMode: true })} colors={colors} />
