...
import type { OrganizationMoreStackParamList } from '../../navigation/types';
import type { ThemeOverride } from '../../theme/ThemeContext';
import type { ThemeColors } from '../../theme/colors';
import { fonts } from '../../theme/fonts';
import { useTheme } from '../../theme/useTheme';

type Props = NativeStackScreenProps<OrganizationMoreStackParamList, 'OrganizationMoreHome'>;
...
// Organization mode's own lean More menu - "Switch to Student" is the one
// way back, same as every other mode's, since Organization is reached from
// Profile regardless of which mode you were in before.
export default function OrganizationMoreScreen({ navigation }: Props) {
  ...
  return (
    <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
        <MenuRow icon="book" label="Organization Library" onPress={() => navigation.getParent()?.navigate('OrgLibrary')} colors={colors} />
        <MenuRow icon="key" label="Access Codes" onPress={() => navigation.getParent()?.navigate('AccessCodes')} colors={colors} />
        <MenuRow icon="newspaper" label="Feed & Events" onPress={() => navigation.getParent()?.navigate('OrgFeeds')} colors={colors} />
        <MenuRow icon="musical-notes" label="Find a Performer" onPress={() => navigation.getParent()?.navigate('FindPerformer')} colors={colors} />
        <MenuRow icon="chatbox-ellipses" label="Feedbacks" onPress={() => navigation.navigate('LiveSupport')} colors={colors} />
